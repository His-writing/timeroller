...
#define ZK_ItemMargin 10.0f
#define ZK_ItemPadding 20.0f
...
//    
    ...
    self.zk_itemDefaultColor = ZK_UIColorFromRGBAlpha(102.0f, 102.0f, 102.0f, 1.0f);
//    self.zk_itemDefaultColor = cyanrgb;

    //选中的背景颜色
//    self.itemStyleSelectedColor = ZK_UIColorFromRGBAlpha(34.0f, 211.0f, 145.0f, 1.0f);
    
    
    self.itemStyleSelectedColor = xDeepBlue;
...
//            self.zk_itemSelectedColor = [UIColor redColor];
...
//    self.segmentBackgroundColor = [UIColor redColor];
...
- (void)zk_itemClickByIndex:(NSInteger)index {
    ...
    if ([self.buttonList count] <=index) {
        
        index=[self.buttonList count]-1;
    }
...
//    buttonItem.titleLabel.numberOfLines=0;
    ...
    buttonItem.titleLabel.lineBreakMode = 0;
                         ...
                         completion:^(BOOL finished) {
                             ...
                             [UIView animateWithDuration:
                              0.3 animations:^{
                                  ...
                                  // 移动到中间
                                  if (scrollerWidth > (ZK_ScreenWidth-40) && // Scroller的宽度大于屏幕宽度
                                   
                                      
                                      
                                      
                                      buttonX > (ZK_ScreenWidth-40) / 2.0f - buttonWidth / 2.0f && //按钮的坐标大于屏幕中间位置
                                      scrollerWidth > buttonX + (ZK_ScreenWidth-40) / 2.0f + buttonWidth / 2.0f // Scroller的宽度大于按钮移动到中间坐标加上屏幕一半宽度加上按钮一半宽度
                                      ...
                                      self.contentOffset = CGPointMake(button.frame.origin.x - (ZK_ScreenWidth-40) / 2.0f + button.frame.size.width / 2.0f, 0);
                                      
                                      
                                      
                                      
                                  } else if (buttonX < (ZK_ScreenWidth-40) / 2.0f - buttonWidth / 2.0f) { // 移动到开始
                                      self.contentOffset = CGPointMake(0, 0);
                                      
                                      
                                      
                                  } else if (scrollerWidth - buttonX < (ZK_ScreenWidth-40) / 2.0f + buttonWidth / 2.0f || // Scroller的宽度减去按钮的坐标小于屏幕的一半，移动到最后
                                             buttonX + buttonWidth + ZK_ItemMargin == scrollerWidth) {
                                      if (scrollerWidth > (ZK_ScreenWidth-40)) {
                                          
                                          
                                          
                                          self.contentOffset = CGPointMake(scrollerWidth - (ZK_ScreenWidth-40), 0); // 移动到末尾
